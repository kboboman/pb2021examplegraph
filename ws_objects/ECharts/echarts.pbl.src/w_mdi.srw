...
public subroutine wf_add_ribbonbar_history (string as_name, string as_picturename, string as_tag);RibbonLargeButtonItem 	lr_LargeButton
RibbonMenu 				lr_Menu
...
//Theme
wf_SetEnabled("ComboBox", "Theme", lb_Type)
//GraphType
wf_SetEnabled("ComboBox", "Graph Type", lb_Type)
//ToolBox
wf_SetEnabled("CheckBox", "ToolBox", lb_Type)
...
//Theme
ls_Theme = Lower(wf_GetValue("ComboBox", "Theme"))
//ToolBox
...
IF li_return = 1 THEN
	...
	rbb_1.SetTabButton(lr_Tab.itemhandle, lr_Tab)
END IF
...
If rbb_1.GetItem( ItemHandle, lr_LargeButton ) = 1 Then
	Choose Case Lower(lr_LargeButton.Tag)
		...
		Case "gauge"
			OpenSheet(w_gauge, Parent, 1, Layered!)
		Case "print"
